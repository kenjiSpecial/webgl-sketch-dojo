...
var relMouse = new THREE.Vector2(-9999, -9999);
...
function init(){
    ...
    solver = new Solver1(grid, renderer);//Solver.make(grid, time, windowSize, renderer)
...
window.addEventListener('mousedown', function(){
    ...
    // relMouse.isMouseDown = true;
});

window.addEventListener('mouseup', function(){
    mouse.isMouseDown = false;
    // relMouse.isMouseDown = false;
});
...
window.addEventListener('mousemove', function(event){
    ...
    relMouse.set( event.clientX / window.innerWidth, 1.-event.clientY / window.innerHeight );
});
